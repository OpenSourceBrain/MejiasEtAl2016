...
function [rate,meaninput]=trial(par,Iext,Nareas,Gw)
...
for time=2*dt:dt:triallength
  ...
  %we keep track of the external (i.e. coming from other areas) input:
  %FB
  externalinput(1,i-1,1)=Gw*W(1,2,1,2)*rate(3,i-1,2);
  externalinput(3,i-1,1)=Gw*W(1,2,2,2)*rate(3,i-1,2);
  externalinput(2,i-1,1)=Gw*W(1,2,3,2)*rate(3,i-1,2);
  externalinput(4,i-1,1)=Gw*W(1,2,4,2)*rate(3,i-1,2);
  %FF:
  externalinput(1,i-1,2)=Gw*W(2,1,1,1)*rate(1,i-1,1);
  externalinput(3,i-1,2)=Gw*W(2,1,2,1)*rate(1,i-1,1);
...
%mean inputs:
meaninput=mean(externalinput,2);
